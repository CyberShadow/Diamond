...
/** The garbage collector handle is not exposed in D2. */
pragma(mangle, "_D2gc5proxy3_gcC2gc2gc2GC")
extern __gshared void* D2gc5proxy3_gcC2gc2gc2GC;

alias theGC = D2gc5proxy3_gcC2gc2gc2GC;
...
version (Windows)
    import std.c.windows.windows : VirtualProtect, PAGE_EXECUTE_WRITECOPY;
else
    import std.c.linux.linux : mprotect, PROT_READ, PROT_WRITE, PROT_EXEC;
...
version (MEMLOG)
{
    ...
    void logDword(uint i)
    {
        fwrite(&i, 4, 1, log);
    }

    void logDword(void* i)
    {
        fwrite(&i, 4, 1, log);
    }
    ...
    void logData(void[] d)
    {
        fwrite(d.ptr, d.length, 1, log);
    }

    void logBits(ref GCBits bits)
    {
        logDword(bits.nwords);

        if (bits.nbits) logData(bits.data[1..1+bits.nwords]);
    }
...
static uint calcDist(void* from, void* to)
{
    return cast(ubyte*)to - cast(ubyte*)from;
}

template Hook(TargetType, HandlerType)
{
    __gshared static ubyte[] target;
    __gshared static ubyte[5] oldcode, newcode;
...
__gshared MethodHook!(1, size_t, Gcx*/*, void**/) fullcollectHook;

version (MEMSTOMP)
{
    __gshared CFunctionHook!(2, byte[], TypeInfo, size_t, Array*) arraysetlengthTHook;
    __gshared CFunctionHook!(3, byte[], TypeInfo, size_t, Array*) arraysetlengthiTHook;
}
version (MEMLOG)
{
    __gshared CFunctionHook!(1, Object, ClassInfo) newclassHook;
}
...
final class DiamondGC : gc_t
{
    ...
    override void* malloc(size_t size, uint bits = 0, size_t* alloc_size = null)
    {
        version (MEMLOG_VERBOSE)
            verboseLog();
        auto result = super.malloc(size);
        mallocHandler(size, result);
        return result;
    }

    override void* calloc(size_t size, uint n = 0, size_t* alloc_size = null)
    {
        version (MEMLOG_VERBOSE)
            verboseLog();
        auto result = super.calloc(size, n);
        callocHandler(size * n, result);
        return result;
    }

    override void* realloc(void* p, size_t size, uint bits = 0, size_t* alloc_size = null)
    {
        version (MEMLOG_VERBOSE)
            verboseLog();
        auto result = super.realloc(p, size);
        reallocHandler(size, p, result);
        return result;
    }
}
...
version (MEMLOG)
{
    ...
    version (MEMLOG_CRC32)
    {
        ...
        __gshared uint*[MAX_POOLS] poolCRCs;
    ...
    extern(C) public void logMemoryDump(bool dataDump, Gcx* gcx = null)
    {
        synchronized (logsync)
        {
            ...
            // log stack
            void* stackTop    = esp();
            void* stackBottom = thread_stackBottom();
            logDword(stackTop);
            ...
            for (int pn = 0; pn < gcx.npools; pn++)
            {
                ...
                // logDword(p.ncommitted); // todo: missing in D2
...
shared static this()
{
    ...
    *cast(void**)theGC = DiamondGC.classinfo.vtbl.ptr;
